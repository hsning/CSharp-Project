    ...
    public partial class Demographic : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["NewlyCreatedDemo"] != null && Session["demographic_id"] != null)
                {
                    ...
                    {
                        StatusLabel.Text = "Success. A new demographic was successfully added to the database. Your study id is: " + Session["demographic_id"].ToString();
                    }
                }
                ...
                edit = false;
                if (user.getDemoAdded(Session["id"].ToString()))
                {
                    DemoPanel.Visible = true;
                    FillData();
                }
        ...
        protected void SubmitBtn_Click(object sender, EventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                ...
                {
                    Session["demographic_id"] = studyID;
        ...
        private bool Submit()
        {
            ...
            studyID = studyManager.GetStudyIDByPatientID(Session["id"].ToString());
            result["patient_id"] = Session["id"];
            result["changed_on"] = DateTime.Now;
            result["changed_by"] = Session["id"];
            result["study_id"] = studyID;
            if (edit)
            {
                result["id"] = username;
            ...
            {
                postResult = studyManager.UpdateVisit(result);
                user.updateDemoAdded(Session["id"].ToString());
            }
        ...
        {
            Label1.Text = "View Demographic (Study ID: " + studyManager.GetStudyIDByPatientID(Session["id"].ToString()) + ")";
            ...
            foreach (DataRow dr in dv.Table.Rows)
            {
                ...
                SexRadioButtonList.SelectedValue = dr[12].ToString();
                ...
                //LocationDDL.BackColor = Color.FromArgb(204, 204, 204);
                BMITextbox.BackColor = Color.FromArgb(235, 235, 228);
                edit = true;
                underBPPanel.Visible = true;
                SubmitBtn.Visible = false;
                Check();
            }
            DemoPanel.Enabled = false;
            RiskFactorsPanel.Enabled = false;
